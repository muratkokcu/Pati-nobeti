...
export default function PlansScreen() {
  ...
  async function setReminder() {
    if (!reminderPlan) { setReminderState('denied'); return; }
  ...
  return <Screen><DisplayText style={styles.heading}>Bakım planları</DisplayText><BodyText style={styles.intro}>Ne yapılacağını değil, ne zaman kayıt beklendiğini birlikte görün.</BodyText>{snapshot.isDemo ? <DemoBanner /> : null}{snapshot.plans.map((plan) => <View key={plan.id} style={styles.card}><View style={styles.time}><Text style={styles.timeText}>{plan.times[0]}</Text></View><View style={styles.copy}><TitleText>{plan.title}</TitleText><BodyText>{plan.instruction}</BodyText><MetaText style={styles.state}>Her gün · {plan.timezone} · aktif</MetaText></View></View>)}{reminderPlan ? <Pressable accessibilityRole="button" onPress={setReminder} style={styles.reminder}><Text style={styles.reminderText}>{reminderState === 'scheduled' ? `${reminderTime} hatırlatıcısı kuruldu` : `${reminderTime} için yerel hatırlatıcı kur`}</Text></Pressable> : <MetaText style={styles.denied}>Hatırlatıcı kurmak için önce bir bakım planı gerekiyor.</MetaText>}{reminderState === 'denied' && reminderPlan ? <MetaText style={styles.denied}>Bildirim izni verilmedi. Cihaz ayarlarından daha sonra açabilirsin.</MetaText> : null}{reminderState === 'timezone' && <MetaText style={styles.denied}>Cihaz saat dilimi planın saat diliminden farklı. Yanlış saatte uyarmamak için yerel hatırlatıcı kurulmadı.</MetaText>}</Screen>;
}
const styles = StyleSheet.create({ heading: { marginTop: spacing.xl }, intro: { color: palette.muted, marginTop: spacing.sm }, card: { backgroundColor: palette.surface, borderColor: palette.line, borderRadius: radius.lg, borderWidth: 1, flexDirection: 'row', gap: spacing.lg, marginBottom: spacing.md, padding: spacing.lg }, time: { alignItems: 'center', backgroundColor: palette.primarySoft, borderRadius: radius.sm, height: touchTarget, justifyContent: 'center', width: layout.railWidth }, timeText: { ...typography.clock, color: palette.primary }, copy: { flex: 1, gap: spacing.xs }, state: { color: palette.primary, marginTop: spacing.sm }, reminder: { alignItems: 'center', borderColor: palette.primary, borderRadius: radius.md, borderWidth: 1, justifyContent: 'center', marginTop: spacing.md, minHeight: touchTarget, paddingHorizontal: spacing.md }, reminderText: { ...typography.bodyStrong, color: palette.primary }, denied: { color: palette.overdue, marginTop: spacing.sm } });
